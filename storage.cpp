...
/*storage::storage(std::vector<uint32_t>& data) : small(false), size_(data.size()) {
    dynamic_buffer = new buffer(data);
}*/
...
bool storage::equals(const storage& rhs) {
...
/*void storage::swap(storage& storage) {
...
void storage::reverse() {
    if (small) {
        if (size_ == 3) { std::swap(static_buffer[0], static_buffer[3]); }
        if (size_ == 2) { std::swap(static_buffer[0], static_buffer[1]); }
    } else {
